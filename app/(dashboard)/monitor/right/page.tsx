...
export default function GateMonitorRightPage() {
  ...
  useEffect(() => {
    if (socket) {
      ...
      socket.on('attendance:scan_right', (data: any) => {
        if (data) {
    ...
    return () => {
      if (socket) {
        ...
        socket.off('attendance:scan_right');
      }
    };
  }, []);
      ...
      <div className="flex items-center justify-between bg-slate-900 p-6 rounded-3xl border border-slate-800 shadow-xl">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-orange-500/20 text-orange-500 border border-orange-500/30 flex items-center justify-center font-black">
            <Icon icon="mingcute:arrow-right-line" className="text-2xl" />
          </div>
          <div>
            <h1 className="text-2xl font-black tracking-tight flex items-center gap-3">
              Kiosk Presensi Gerbang KANAN (Right Gate)
            ...
            <p className="text-xs text-slate-400 font-semibold mt-0.5">
              Monitor Real-Time Hasil Scan Wajah & Gate Terminal Kanan · SMK SMTI Yogyakarta
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {logs.length > 0 ? (
          ...
          <div className="col-span-full p-16 text-center text-slate-500 font-semibold bg-slate-900/50 rounded-3xl border border-slate-800">
            Menunggu event presensi scan wajah dari Gerbang Kanan...
          </div>
        )}
      </div>
    </div>
